...
function buildDeterministicMemo(
  ...
  return {
    ...
    factsHash: hash,
    model: "rules",
    executiveSummary: `${snapshot.profile.name} 是一家 ${snapshot.profile.sector} 公司，当前基本面质量为${quality ? scoreLabelZh(quality.label) : "分歧"}，估值为${valuation ? scoreLabelZh(valuation.label) : "分歧"}，预期动量为${expectations ? scoreLabelZh(expectations.label) : "分歧"}。这不是买卖建议；更适合从现金创造、预期修正、估值偏离、事件催化和行为披露之间的关系来研究。`,
    whatChanged: {
