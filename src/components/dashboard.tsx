          ...
          <div className="flex flex-col justify-between gap-6 rounded-md border border-line bg-white p-6 shadow-sm">
            <div>
              ...
              <p className="mt-3 text-sm font-medium text-steel">
                当前研究宇宙来自观察列表：{model.universe.count} 家公司。
              </p>
            </div>
            <SearchBox />
          </div>
        ...
        <section className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="rounded-md border border-line bg-white p-5 shadow-sm">
            ...
            {model.watchlist.length ? (
              ...
              <EmptyState
                title="观察列表还是空的"
                text="先添加你真正想研究的股票，首页、选股器、市场和日历会自动基于这批标的生成。"
              />
            )}
          </div>
          ...
          <div className="rounded-md border border-line bg-white p-5 shadow-sm">
            ...
            {model.researchIdeas.length ? (
              ...
              <EmptyState
                title="暂无研究候选"
                text="研究候选会从观察列表里的公司信号自动生成，不再使用固定示例。"
              />
            )}
          </div>
        </section>
...
function EmptyState({ title, text }: { title: string; text: string }) {
  return (
    <div className="mt-4 rounded-md border border-dashed border-line bg-canvas p-5">
